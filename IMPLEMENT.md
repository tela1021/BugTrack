...
### 2026-07-22 — P1/P2 / workflows, accessibility and management

Список получил фильтры проекта, цикла, типа, метки и периода обновления с URL-синхронизацией, тип и цикл как настраиваемые колонки и сохранение ширины колонок в профиле. Карточка задачи поддерживает безопасный Markdown-preview, проект, цикл, метки, тип, родительскую задачу и подзадачи; обсуждение и history отображаются от старых записей к новым. Kanban получил sticky-заголовки и доступное перемещение через «Переместить в…» с существующей WIP-проверкой.

Добавлены mobile drawer, адаптивные фильтры/detail, skip link и системная/светлая/тёмная тема с хранением настройки в профиле. В P2 добавлены lifecycle циклов (goal/status/единственный active), командный dashboard, SLA-summary для bug по business-time/timezone, рабочие PostgreSQL-индексы, а также GitHub webhook с HMAC-проверкой, идемпотентным delivery log и обработкой `Fixes`/`Closes` для настроенной интеграции.

**Verification:** `npm test`: 77/77 passed; `npm run typecheck`, `npm run lint`, `npm run build`, `git diff --check`: passed. Локальный PostgreSQL `127.0.0.1:5434` принимает подключения; `prisma migrate status` подтверждает 13 применённых migrations.

### 2026-07-22 — регрессия формы создания задачи

Форма не отправляет запрос до загрузки команды и её статусов: вместо пустых списков отображается явное состояние загрузки, а кнопка создания недоступна. Ошибки Zod на сервере преобразуются из технического JSON в короткие сообщения для пользователя, в том числе «Выберите статус задачи».

**Verification:** RED → GREEN regression-contract, `npm test` (83/83), `npm run typecheck`, `npm run lint`, `npm run build` и `git diff --check`: passed.
...
### 2026-07-27 — project controls and dashboard scrolling

**Planned:**

Добавить явное состояние сохранения при смене проекта в карточке задачи, разнести фильтры команды и проекта по понятным подписям и заменить фиксированную высоту Kanban на область, занимающую доступную высоту и прокручиваемую по обеим осям.

**Implemented:**

- В карточке задачи проект меняется отдельным обработчиком; повторный ввод блокируется на время сохранения, результат подтверждается toast, а отсутствие проектов объясняется рядом с селектом.
- Фильтры «Команда» и «Проект» получили разные доступные имена и корректные варианты «Все команды»/«Все проекты»; существующий `projectId` в URL и server-side filtering сохранены.
- Dashboard получил flex-layout на доступную высоту. Kanban использует одну focusable область с горизонтальной и вертикальной прокруткой, общей вертикальной лентой и видимой подсказкой про колесо/Shift+колесо.
- Добавлен `test/project-dashboard-usability.test.mjs`; изменённые файлы ограничены dashboard, фильтрами, issue detail, тестом и harness-журналами.

**Verification:**

RED: 3/3 новых теста падали на отсутствующих контрактах. GREEN: targeted 7/7, полный `npm test` 87/87, `npm run typecheck`, `npm run lint`, `npm run build`, `git diff --check` прошли. Локальный dev-server и `/login` открылись в браузере. Authenticated UI не проверен вручную: настроенная PostgreSQL `127.0.0.1:5434` недоступна; временно запущенная системная PostgreSQL на 5432 не соответствует конфигурации и была остановлена без изменения данных.

### 2026-07-29 — изменение порядка workflow-статусов

**Implemented:**

- В `/admin/workflow` статусы переставляются за отдельную drag-handle с pointer- и keyboard-сенсорами `dnd-kit`.
- Новый порядок применяется оптимистично, автоматически сохраняется и откатывается при ошибке; сохранение сопровождается `aria-live` и toast-обратной связью.
- `reorderWorkflowStatuses` проверяет права администратора, валидирует полный уникальный набор статусов выбранной команды и сохраняет позиции двухфазно в одной транзакции, не нарушая уникальность `(teamId, position)`.
- Поскольку dashboard уже запрашивает статусы с сортировкой по `position`, сохранённый порядок определяет порядок Kanban-колонок без миграции схемы.
- Подтверждённый дизайн записан в `docs/plans/2026-07-29-workflow-status-reordering-design.md`; регрессии покрыты `test/workflow-status-reordering.test.mjs` и обновлённым RBAC-контрактом.

**Verification:**

RED: 2/2 новых теста падали на отсутствующих server action и DnD-контрактах. GREEN: целевые 2/2 и полный `npm test` 89/89 прошли; `npm run typecheck`, `npm run lint`, `npm run build` и `git diff --check` прошли. Конфигурация Prisma валидна, PostgreSQL слушает `127.0.0.1:5434`, но прямой запрос из sandbox получил `P1001`, а повышение прав было отклонено внутренней ошибкой OpenAI. Поэтому сохранение в реальную БД и authenticated UI вручную не проверены.
